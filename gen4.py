...
class MBar:
    '''the data structure to hold the one music bar
    '''
    def __init__(self, timing=(4,4), notes=[]):
        self.timing = timing
        self.beats, self.perBeats = timing
        self.notes = notes

class MNote:
    ''' the data structure to hold one music note
    '''
    def __init__(self, pitch='c', duration='4', note='c4'):
        if node:
            self.pitch, self.duration = _split(note)
            if None in (self.pitch, self.duration):
                log.error("Error in note: %s" % note)
            elif not None in (pitch, duration):
                self.pitch = pitch
                self.duration = duration
                self.time = getTime(duration)
    def getTime(duration):
        m = re.search('(1|2|4|8|16|32)\.?', duration)
        if not m:
